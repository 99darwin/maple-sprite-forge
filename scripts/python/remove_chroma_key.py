...
#
# remove_chroma_key.py — solid-color matte to alpha.
#
# Vendored into maple-sprite-forge from the imagegen skill in the open-source
# codex skills tree. Kept Python (Pillow) rather than ported to Node/sharp
# because the matte/despill/edge-feather logic is subtle and a faithful copy
# is more useful than a half-equivalent rewrite.
...
Use this between the model output and the normalizer.
"""
...
def _load_pillow():
    try:
        ...
        _die(
            "Pillow is required. Install it with `pip install -r "
            "scripts/python/requirements.txt` (or `pip install pillow`)."
        )
